...
class Solution:
    ...
    # Alternative: Instead of storing the current level, process level by level (more memory efficient, slightly less readable in my opinion but I actually prefer this solution)
    def levelOrder(self, root: Optional[TreeNode]) -> List[List[int]]:
        q = deque()
        q.append(root)
        res = []

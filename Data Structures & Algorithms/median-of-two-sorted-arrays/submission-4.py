...
# Notes
# - Using -inf/inf can sometimes be significantly more readable than None
...
class Solution:
    ...
    # Improved Readability using -inf/inf
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        small, big = (nums1, nums2) if len(nums1) < len(nums2) else (nums2, nums1)
        n, m = len(small), len(big)
        half = (n + m + 1) // 2
        ...
        while l <= r:
            s = (l + r) // 2
            s_el = float('-inf') if s == 0 else small[s - 1]
            s_next = float('inf') if s >= n else small[s]

            b_el = float('-inf') if half - s <= 0 else big[half - s - 1]
            b_next = float('inf') if half - s >= m else big[half - s]
            ...
            if s_next < b_el:
                ...
                res = s_el if s_el > b_el else b_el
                if (n + m) % 2 == 0:
                    compl = s_next if s_next < b_next else b_next
                    res = (res + compl) / 2
                return res
